...
// We now know that we are inside of a week, so we'll draw the table row (<tr>) for this week and begin draw.
       ...
       foreach($week as $d) {
         ...
         if($i < $offset_count){
				...
				$calDays = mysqli_query($brewing,$query_calDays) or die(mysqli_error($brewing));
			 ...
			 echo "<table class=\"none\">";
			   ...
			   mysqli_select_db($brewing,$database_brewing);
				...
				$upDays = mysqli_query($brewing,$query_upDays) or die(mysqli_error($brewing));
			   ...
			   do {
				...
				$upRecipe  = mysqli_query($brewing,$query_upRecipe) or die(mysqli_error($brewing));
