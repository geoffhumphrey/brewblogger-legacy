...
$brewing = mysqli_query($brewing,$query_brewing) or die(mysqli_error($brewing));
...
$query_styles = "SELECT * FROM styles ORDER BY brewStyleVersion,brewStyleGroup,brewStyleNum ASC";
$styles = mysqli_query($brewing,$query_styles) or die(mysqli_error($brewing));
$row_styles = mysqli_fetch_assoc($styles);
$totalRows_styles = mysqli_num_rows($styles);

$query_links = "SELECT * FROM brewerlinks ORDER BY brewerLinkName ASC";
$links = mysqli_query($brewing,$query_links) or die(mysqli_error($brewing));
$row_links = mysqli_fetch_assoc($links);
$totalRows_links = mysqli_num_rows($links);

$query_users = "SELECT * FROM users ORDER BY realLastName ASC";
$users = mysqli_query($brewing,$query_users) or die(mysqli_error($brewing));
...
$extracts = mysqli_query($brewing,$query_extracts) or die(mysqli_error($brewing));
...
$misc = mysqli_query($brewing,$query_misc) or die(mysqli_error($brewing));
...
$yeast_profiles = mysqli_query($brewing,$query_yeast_profiles) or die(mysqli_error($brewing));
...
$adjuncts = mysqli_query($brewing,$query_adjuncts) or die(mysqli_error($brewing));
...
$hops = mysqli_query($brewing,$query_hops) or die(mysqli_error($brewing));
$row_hops = mysqli_fetch_assoc($hops);
$totalRows_hops = mysqli_num_rows($hops); 

$query_grains = "SELECT * FROM malt ORDER BY maltName ASC";
$grains = mysqli_query($brewing,$query_grains) or die(mysqli_error($brewing));
...
$sugar_type = mysqli_query($brewing,$query_sugar_type) or die(mysqli_error($brewing));
$row_sugar_type = mysqli_fetch_assoc($sugar_type);
$totalRows_sugar_type = mysqli_num_rows($sugar_type);
}
...
if ((isset($_GET['id'])) && ($_GET['id'] != "")) {
  ...
  mysqli_select_db($brewing,$database_brewing);
  $Result1 = mysqli_query($brewing,$deleteSQL) or die(mysqli_error($brewing));
  }
...
$log2 = mysqli_query($brewing,$query_log2) or die(mysqli_error($brewing));
