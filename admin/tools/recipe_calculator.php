...
$recipeRecalc = mysqli_query($brewing,$query_recipeRecalc) or die(mysqli_error($brewing));
$row_recipeRecalc = mysqli_fetch_assoc($recipeRecalc);
}
   			...
   			<select name="brewStyle">
    			<?php do {  ?>
     			<option value="<?php echo $row_styles['brewStyle']?>" <?php if ($results == "true") { if ($brewStyle == $row_styles['brewStyle']) echo "SELECTED"; } if ((($source == "recipes") || ($source == "brewing")) && ($results == "false")) { if ($row_recipeRecalc['brewStyle'] == $row_styles['brewStyle']) echo "SELECTED" ; } ?>><?php echo $row_styles['brewStyle'];?></option>
      			<?php } while ($row_styles = mysqli_fetch_assoc($styles)); $rows = mysqli_num_rows($styles); if($rows > 0) { mysqli_data_seek($styles, 0); $row_styles = mysqli_fetch_assoc($styles); } ?>
   			</select>
   		</td>
   	</tr>
</table>
	...
	<tr>
		...
		<td class="data" width="5%">
   			<select name="brewExtract1" id="brewExtract1">
    			...
    			<?php } while ($row_extracts = mysqli_fetch_assoc($extracts)); $rows = mysqli_num_rows($extracts); if($rows > 0) { mysqli_data_seek($extracts, 0); $row_extracts = mysqli_fetch_assoc($extracts); } ?>
   			</select>
		</td>
	...
	<tr>
   		...
   		<td class="data" width="5%">
   			<select name="brewGrain1" id="brewGrain1">
    			...
    			<?php } while ($row_grains = mysqli_fetch_assoc($grains)); $rows = mysqli_num_rows($grains);	if($rows > 0) {	mysqli_data_seek($grains, 0); $row_grains = mysqli_fetch_assoc($grains); } ?>
   			</select>
   		</td>
   		<td class="dataLabel">Weight:</td>
	...
	<tr>
		...
		<td class="data" width="5%">
   			<select name="brewAdjunct1" id="brewAdjunct1">
    			...
    			<?php } while ($row_adjuncts = mysqli_fetch_assoc($adjuncts)); $rows = mysqli_num_rows($adjuncts); if($rows > 0) { mysqli_data_seek($adjuncts, 0); $row_adjuncts = mysqli_fetch_assoc($adjuncts); } ?>
   			</select>
            </td>
		<td class="dataLabel">Weight:</td>
