...
<tr>
     ...
     <?php } while ($row_users = mysqli_fetch_assoc($users)); $rows = mysqli_num_rows($users); if($rows > 0) { mysqli_data_seek($users, 0); $row_users = mysqli_fetch_assoc($users); } ?>
     </select>
     </td>
</td>
<?php } ?>
...
<tr>
    ...
    <?php do {  ?>
     <option value="<?php echo $row_recipes['id']?>" <?php if ($action == "edit") {  if (($row_log['upcomingRecipeID']) == ($row_recipes['id'])) {echo "SELECTED";} }?>><?php echo $row_recipes['brewName']?></option>
      <?php } while ($row_recipes = mysqli_fetch_assoc($recipes)); $rows = mysqli_num_rows($recipes); if($rows > 0) { mysqli_data_seek($recipes, 0); $row_styles = mysqli_fetch_assoc($recipes); } ?>
